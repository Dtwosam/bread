...
export function registerFeedRoute(app: FastifyInstance, deps: BreadFeedRouteDeps): void {
  app.get("/v1/feed", async (request, reply) => {
    ...
    try {
      ...
      return reply.code(400).send({ error: { code: "INVALID_HOLDER_FILTER", message: "Holder filter is invalid.", requestId: request.id } });
    }
    ...
    try {
      ...
      return reply.code(400).send({ error: { code: "INVALID_PROGRESS_FILTER", message: "Baked progress filter is invalid.", requestId: request.id } });
    }
    ...
    try {
      ...
      return reply.code(400).send({ error: { code: "INVALID_VOLUME_FILTER", message: "24h volume filter is invalid.", requestId: request.id } });
    }

    let marketCapBounds: ReturnType<typeof parseExploreMarketCapBounds>;
    try {
      marketCapBounds = parseExploreMarketCapBounds(query.marketCapMinQuote, query.marketCapMaxQuote);
    } catch {
      return reply.code(400).send({ error: { code: "INVALID_MARKET_CAP_FILTER", message: "Market cap filter is invalid.", requestId: request.id } });
    }
    ...
    if (query.creator !== undefined) {
      try {
        ...
        return reply.code(400).send({ error: { code: "INVALID_CREATOR_FILTER", message: "Creator wallet filter is invalid.", requestId: request.id } });
      }
    }
    ...
    const load = async () => {
      const feedMeta =
        view === "trending" || view === "graduating" || ageFilter !== undefined || volumeBounds !== undefined
          ? await deps.freshness()
          : undefined;
      const ageBounds = ageFilter === undefined
        ? undefined
        : resolveExploreAgeBounds(feedMeta!.indexedThroughBlockTimestamp, ageFilter);
      const hasProjectionFilters =
        ageBounds !== undefined || holderBounds !== undefined || progressBounds !== undefined || creatorAddress !== undefined || volumeBounds !== undefined || marketCapBounds !== undefined;
      ...
      if (hasMore && last) {
        if (view === "graduated") {
          ...
          if (!("quoteVolume1h" in last) || !("uniqueTraders1h" in last) || !("tradeCount1h" in last) || !("latestActivityBlockNumber" in last) || !("latestActivityLogIndex" in last)) {
          ...
          if (typeof quoteVolume1h !== "bigint" || typeof uniqueTraders1h !== "bigint" || typeof tradeCount1h !== "bigint" || typeof latestActivityBlockNumber !== "bigint" || typeof latestActivityLogIndex !== "number") {
    ...
    return {
      ...cacheResult.value,
      meta: { ...cacheResult.value.meta, servedAt: now.toISOString(), cache: cacheResult.cache },
    };
  });
}
