...
export function serializeCurveState(row: NonNullable<LaunchStateRow> | undefined) {
  ...
  return {
    ...
    poolId: row.poolId,
    graduationAdapter: row.graduationAdapter,
    sweptUsdcAmount: row.sweptUsdcAmount?.toString(10) ?? null,
    sweptTokenAmount: row.sweptTokenAmount?.toString(10) ?? null,
    graduationFailureReasonHash: row.graduationFailureReasonHash,
    positionManager: row.positionManager,
    positionId: row.positionId?.toString(10) ?? null,
    usdcUsed: row.usdcUsed?.toString(10) ?? null,
    tokenUsed: row.tokenUsed?.toString(10) ?? null,
    tokenLocked: row.tokenLocked?.toString(10) ?? null,
    usdcDust: row.usdcDust?.toString(10) ?? null,
    positionLocked: row.positionLocked,
    tokenSupplyLocked: row.tokenSupplyLocked?.toString(10) ?? null,
    graduationCompletedBlock: row.graduationCompletedBlock?.toString(10) ?? null,
