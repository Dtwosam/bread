...
function isGraduated(phase: string | null, state: string | null): boolean {
  return phase === 'POOL_CREATED' || state === 'POOL_CREATED' || state === 'GRADUATED';
}

export function registerPortfolioRoute(app: FastifyInstance, deps: BreadReadRouteDeps): void {
  app.get('/v1/portfolio/:address', async (request, reply) => {
    ...
    try {
      ...
      return reply.code(400).send({
        error: { code: 'INVALID_ADDRESS', message: 'Wallet address is invalid.', requestId: request.id },
      });
    }

    const query = request.query as { limit?: string; cursor?: string };
    const parsedLimit = query.limit === undefined ? DEFAULT_PORTFOLIO_LIMIT : Number(query.limit);
    if (!Number.isInteger(parsedLimit) || parsedLimit < 1 || parsedLimit > MAX_PORTFOLIO_LIMIT) {
      return reply.code(400).send({
        error: {
          code: 'INVALID_LIMIT',
    ...
    const nextCursor = hasMore && last
      ? encodePortfolioCursor({ version: PORTFOLIO_CURSOR_VERSION, tokenAddress: last.tokenAddress })
      : undefined;
    ...
    return {
      data: {
        ...
        holdings: rows.map((row) => {
          const graduated = isGraduated(row.graduationPhase, row.graduationState);
          ...
          const price = graduated
            ? { status: 'UNAVAILABLE' as const, reason: 'NO_RATIFIED_LIVE_DEX_PRICE_SOURCE' as const }
            : hasIndexedCurvePrice
              ? {
              ...
              : { status: 'UNAVAILABLE' as const, reason: 'NO_INDEXED_CURVE_PRICE' as const };

          const currentValue = hasIndexedCurvePrice
            ? {
                status: 'AVAILABLE' as const,
                source: 'INDEXED_CURVE_EXECUTION' as const,
                numerator: (BigInt(row.balance) * BigInt(row.lastPriceNumerator as string)).toString(10),
                denominator: row.lastPriceDenominator as string,
              }
            : {
                status: 'UNAVAILABLE' as const,
                reason: graduated ? 'NO_RATIFIED_LIVE_DEX_PRICE_SOURCE' as const : 'NO_INDEXED_CURVE_PRICE' as const,
              };
