...
export function TokenIdentity({
  token,
}: Readonly<{ token: IndexedTokenDetail }>) {
  const metadata = token.metadata;
  const initial = (token.symbol?.trim() || token.name?.trim() || "?")
    .slice(0, 1)
    .toUpperCase();
  const links = [
  ...
  return (
    <section
      className="bread-token-identity"
      aria-labelledby="bread-token-name"
    >
      ...
      <div className="bread-token-identity__copy">
        ...
        {metadata.description ? (
          <p className="bread-token-identity__description">
            {metadata.description}
          </p>
        ) : null}
        {links.length > 0 ? (
        ...
        <dl className="bread-token-identity__meta">
          ...
          <div>
            ...
            <dd>
              <CreatorAttribution
                creatorAddress={token.deployerAddress}
                size="token-header"
              />
            </dd>
          </div>
