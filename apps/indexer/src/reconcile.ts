...
export type ReconciliationChainReader = Readonly<{
  ...
  ) => Promise<readonly LaunchCreatedIdentity[]>;
  scanCanonicalEventIdentities: (
    input: Readonly<{ fromBlock: bigint; toBlock: bigint }>,
  ) => Promise<readonly CanonicalEventIdentity[]>;
  readCurveState: (
    input: Readonly<{
      tokenAddress: string;
      curveAddress: string;
  ...
  >;
  readGraduationState: (
    input: Readonly<{ tokenAddress: string; blockNumber: bigint }>,
  ) => Promise<
    Readonly<{
      phase: string;
...
export type RebuildStackInput = Readonly<{
  ...
  chain?: ReconciliationChainReader;
  verifyRebuildTarget?: () =>
    Promise<RebuildTargetVerification> | RebuildTargetVerification;
...
function stateByToken(
  states: readonly ReconciliationLaunchStateRow[],
): Map<string, ReconciliationLaunchStateRow> {
  return new Map(
    states.map((state) => [state.tokenAddress.toLowerCase(), state]),
  );
}
...
function optionalAddressMatches(
  projected: string | null | undefined,
  authoritative: string | null | undefined,
): boolean {
  if (authoritative === undefined) return true;
  return (
    (projected ?? null)?.toLowerCase() ===
    (authoritative ?? null)?.toLowerCase()
  );
}

function optionalValueMatches<T>(
  projected: T | null | undefined,
  authoritative: T | null | undefined,
): boolean {
  return (
    authoritative === undefined ||
    (projected ?? null) === (authoritative ?? null)
  );
}

export function buildRebuildReport(input: RebuildReportInput): RebuildReport {
  if (
    !Number.isSafeInteger(input.canonicalEventCount) ||
    input.canonicalEventCount < 0
  ) {
    throw new Error(
      "rebuild report canonicalEventCount must be a non-negative safe integer",
    );
  }
  if (!Number.isSafeInteger(input.rangesApplied) || input.rangesApplied < 0) {
    throw new Error(
      "rebuild report rangesApplied must be a non-negative safe integer",
    );
  }
  if (!input.targetIdentity.trim())
    throw new Error("rebuild report targetIdentity is required");
  ...
  const reportHash = sha256(
    stringToHex(JSON.stringify(canonical)),
    "hex",
  ).slice(2);
...
export async function reconcileStack(
  input: ReconcileStackInput,
): Promise<ReconciliationReport> {
  const startedAt = new Date().toISOString();
  if (input.checkedBlock < input.context.deploymentStartBlock)
    throw new Error("reconciliation block precedes deployment start");
  ...
  const [authoritativeLaunchCount, authoritativeLaunchEvents] =
    await Promise.all([
      input.chain.countLaunchCreated(launchRange),
      input.chain.scanLaunchCreated(launchRange),
    ]);
  const expectedLaunches = [...authoritativeLaunchEvents]
    .map(launchIdentityKey)
    .sort();
  const actualLaunches = snapshot.launches
    .map((launch) =>
      launchIdentityKey({
    ...
    .sort();
  const launchIdentityMatch =
    expectedLaunches.length === actualLaunches.length &&
  ...
  for (const launch of snapshot.launches) {
    ...
    const projectedGraduated = projected
      ? projectedCurveGraduated(projected.graduationPhase)
      : null;
    const expected = {
    ...
    if (
      ...
      projected.remainingSellableTokens !==
        authoritative.remainingSellableTokens ||
  ...
  if (feeEscrowAddress) {
    ...
    feePass =
      projectedOutstanding === authoritative.totalOutstanding &&
      authoritative.custody >= authoritative.totalOutstanding;
    feeExpected = {
      totalOutstanding: authoritative.totalOutstanding,
      custodyAtLeast: authoritative.totalOutstanding,
    };
    feeActual = {
      projectedOutstanding,
      onchainTotalOutstanding: authoritative.totalOutstanding,
      custody: authoritative.custody,
    };
  ...
  for (const launch of snapshot.launches) {
    const authoritative = await input.chain.readGraduationState({
      tokenAddress: launch.tokenAddress,
      blockNumber: input.checkedBlock,
    });
    const projected = states.get(launch.tokenAddress.toLowerCase());
    const projectedVenue = graduatedVenues.get(
      launch.tokenAddress.toLowerCase(),
    );
    const authoritativeVenueKind =
      authoritative.graduatedVenueKind ?? authoritative.venueKind;
    const authoritativeVenueAddress =
      authoritative.graduatedVenueAddress ?? authoritative.venueAddress;
    const authoritativeVenueFeeTier =
      authoritative.graduatedVenueFeeTier ?? authoritative.venueFeeTier;
    const authoritativeQuoteIsToken0 =
      authoritative.graduatedVenueQuoteIsToken0 ?? authoritative.quoteIsToken0;
    ...
    graduationActual.push({
      ...
      graduatedVenueQuoteIsToken0:
        projectedVenue?.graduatedVenueQuoteIsToken0 ?? null,
      graduationCompletedBlock:
        projectedVenue?.graduationCompletedBlock ?? null,
      graduationCompletedTransactionIndex:
        projectedVenue?.graduationCompletedTransactionIndex ?? null,
      graduationCompletedLogIndex:
        projectedVenue?.graduationCompletedLogIndex ?? null,
    });
    if (
      !projected ||
      ...
      (projected.poolId ?? null)?.toLowerCase() !==
        (authoritative.poolId ?? null)?.toLowerCase() ||
      ...
      !optionalValueMatches(
        projectedVenue?.graduatedVenueKind,
        authoritativeVenueKind,
      ) ||
      !optionalAddressMatches(
        projectedVenue?.graduatedVenueAddress,
        authoritativeVenueAddress,
      ) ||
      !optionalValueMatches(
        projectedVenue?.graduatedVenueFeeTier,
        authoritativeVenueFeeTier,
      ) ||
      !optionalValueMatches(
        projectedVenue?.graduatedVenueQuoteIsToken0,
        authoritativeQuoteIsToken0,
      ) ||
      !optionalValueMatches(
        projectedVenue?.graduationCompletedBlock,
        authoritative.graduationCompletedBlock,
      ) ||
      !optionalValueMatches(
        projectedVenue?.graduationCompletedTransactionIndex,
        authoritative.graduationCompletedTransactionIndex,
      ) ||
      !optionalValueMatches(
        projectedVenue?.graduationCompletedLogIndex,
        authoritative.graduationCompletedLogIndex,
      )
    )
  ...
  let codeHashPass = Boolean(
    snapshot.stack && expectedHashes && registeredEntries.size > 0,
  );
  for (const [role, target] of [...registeredEntries.entries()].sort(
    ([a], [b]) => a.localeCompare(b),
  )) {
  ...
  const configPass = Boolean(
    ...
    chainConfig.quoteAsset.toLowerCase() ===
      input.context.quoteAsset.toLowerCase() &&
    chainConfig.quoteDecimals === input.context.quoteDecimals &&
    snapshot.stack.quoteAsset.toLowerCase() ===
      input.context.quoteAsset.toLowerCase() &&
  ...
  const observedCheckpointHash = await input.chain.getBlockHash(
    input.checkedBlock,
  );
  const authoritativeEventIdentities = (
    await input.chain.scanCanonicalEventIdentities({
      fromBlock: input.context.deploymentStartBlock,
      toBlock: input.checkedBlock,
    })
  )
  ...
  const projectedEventIdentities = snapshot.journal
    ...
    .map(eventIdentityKey)
    .sort();
  const journalIdentityMatch =
    authoritativeEventIdentities.length === projectedEventIdentities.length &&
    authoritativeEventIdentities.every(
      (value, index) => value === projectedEventIdentities[index],
    );
  const journalWithinCheckpoint =
    Boolean(checkpoint) &&
    snapshot.journal.every(
      (item) =>
  ...
  const journalDecoderMatch = snapshot.journal.every(
    (item) => item.decoderSchemaVersion === DAY6_DB_SCHEMA_VERSION,
  );
  const observedJournalDecoderVersions = [
    ...new Set(snapshot.journal.map((item) => item.decoderSchemaVersion)),
  ].sort();
  const checkpointPass = Boolean(
    checkpoint &&
    snapshot.stack &&
    checkpoint.deploymentStartBlock === input.context.deploymentStartBlock &&
    snapshot.stack.deploymentStartBlock ===
      input.context.deploymentStartBlock &&
    checkpoint.indexedThroughBlock === input.checkedBlock &&
    checkpoint.indexedThroughBlockHash.toLowerCase() ===
      observedCheckpointHash.toLowerCase() &&
...
export async function rebuildStack(
  input: RebuildStackInput,
): Promise<RebuildStackResult> {
  if (input.targetBlock < input.context.deploymentStartBlock)
    throw new Error("rebuild target precedes deployment start");
  if (input.batchSize <= 0n)
    throw new Error("rebuild batchSize must be positive");
  if (input.skipReconciliation || !input.chain) {
    throw new Error(
      "authoritative chain reader is required after rebuild; reconciliation cannot be skipped",
    );
  }
  if (!input.verifyRebuildTarget) {
    throw new Error(
      "isolated rebuild target verification is required before destructive rebuild",
    );
  }
  const target = await input.verifyRebuildTarget();
  if (
    (target.targetMode !== "ISOLATED_REPLACEMENT" &&
      target.targetMode !== "LOCAL_TEST") ||
    !target.targetIdentity.trim()
  ) {
    throw new Error(
      "isolated rebuild target verification returned an invalid target",
    );
  }
  ...
  while (fromBlock <= input.targetBlock) {
    const toBlock =
      fromBlock + input.batchSize - 1n > input.targetBlock
        ? input.targetBlock
    ...
    if (loaded.fromBlock !== fromBlock || loaded.toBlock !== toBlock) {
      throw new Error(
        "rebuild loader returned a range different from the requested contiguous batch",
      );
    }
    await applyRange({
      db: input.db,
      ...
      ...(loaded.toBlockTimestamp === undefined
        ? {}
        : { toBlockTimestamp: loaded.toBlockTimestamp }),
      logs: loaded.logs,
    });
    rangesApplied += 1;
  ...
  if (reconciliation.status !== "PASS") {
    const failedChecks = reconciliation.checks
      ...
      .join(",");
    throw new Error(
      `authoritative reconciliation failed: ${failedChecks || "UNKNOWN"}`,
    );
  }
