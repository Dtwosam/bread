...
export type IndexerCliResolver = (selection: IndexerCliSelection) => Promise<IndexerCommandRequest> | IndexerCommandRequest;
 ...
 */
export function parseIndexerCliArgs(argv: readonly string[]): IndexerCliSelection {
  ...
  for (let index = 0; index < args.length; index += 2) {
    ...
    switch (flag) {
      case '--network':
        if (network !== undefined) throw new Error('duplicate --network argument');
        network = value;
        break;
      case '--stack':
        if (stackVersion !== undefined) throw new Error('duplicate --stack argument');
        stackVersion = value;
        break;
      default:
...
export async function runIndexerCli<T = IndexerCommandResult>(
  ...
  execute: (request: IndexerCommandRequest) => Promise<T> = runIndexerCommand as (request: IndexerCommandRequest) => Promise<T>,
): Promise<T> {
  const selection = parseIndexerCliArgs(argv);
  const request = await resolve(selection);
  if (request.command !== selection.command) {
    throw new Error(`resolved command ${request.command} does not match requested command ${selection.command}`);
  }
