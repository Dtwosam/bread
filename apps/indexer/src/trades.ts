...
function curveTokenMap(
  ...
  for (const launch of knownLaunches) {
    ...
    if (previous && previous !== token)
      throw new Error(`curve ${curve} maps to multiple launch tokens`);
    result.set(curve, token);
  }
  return result;
}

export function correlateCanonicalTrades(
  input: Readonly<{
  ...
  for (const event of sortEvents(input.events)) {
    ...
    if (event.eventName === "CurveBuy") {
      ...
      const opening = exactlyOne(
        openingMatches,
        "OpeningProtectionApplied context",
      );
      ...
      if (refundMatches.length > 1)
        throw new Error("ambiguous CurveBuyRefunded context");
      const refund = refundMatches[0];
      if (refund)
        state.refunds = state.refunds.filter(
          (candidate) => candidate !== refund,
        );
      ...
      if (charges > spent)
        throw new Error("CurveBuy charges exceed spent quote");
      const netCurveInput = spent - charges;
      if (tokenAmount === 0n)
        throw new Error("CurveBuy token amount must be positive");
    ...
    if (event.eventName === "CurveSell") {
      ...
      if (tokenAmount === 0n)
        throw new Error("CurveSell token amount must be positive");
  ...
  for (const [key, state] of stateByTxCurve) {
    if (state.openings.length > 0)
      throw new Error(`unconsumed OpeningProtectionApplied context in ${key}`);
    if (state.refunds.length > 0)
      throw new Error(`unconsumed CurveBuyRefunded context in ${key}`);
  }
