...
const canonicalToken = address('a');
const feeOnlyToken = address('b');
const canonicalCurve = address('c');
const feeOnlyCurve = address('d');
const factory = address('e');
const creator = address('f');
const otherWallet = address('1');
const indexedHeadTimestamp = 1_786_262_400n;

const context: ProtocolContext = {
  network: 'arc-testnet',
  chainId: 5_042_002,
  stackVersion: 'search-creator-test',
...
describe.skipIf(!RUN_DB)('Day 6 Search canonical creator-wallet authority', () => {
  const schemaName = `day6_search_creator_${process.pid}`;
  ...
  beforeEach(async () => {
    ...
    await pool.query(
      `INSERT INTO launches (
        ...
        ($1,$2,$3,$4,$5,$6,$7,$8,'Canonical Creator','CCRT','1000','120',$9,2),
        ($1,$10,$11,$4,$5,$7,$6,$8,'Fee Recipient Only','FEE','1000','119',$12,1)`,
      [
        context.chainId,
        canonicalToken.toLowerCase(),
        canonicalCurve.toLowerCase(),
        context.stackVersion,
        factory.toLowerCase(),
        creator.toLowerCase(),
        otherWallet.toLowerCase(),
        (indexedHeadTimestamp - 60n).toString(10),
        `0x${'ab'.repeat(32)}`,
        feeOnlyToken.toLowerCase(),
        feeOnlyCurve.toLowerCase(),
        `0x${'cd'.repeat(32)}`,
      ],
    );
  });
  ...
  async function createSearchApp() {
    ...
    route.registerSearchRoute(app as never, {
      ...
      freshness: async () => ({
        ...
        indexedThroughBlockHash: `0x${'ef'.repeat(32)}`,
  ...
  it('matches creator-wallet Search only through canonical deployer identity, never fee-recipient metadata', async () => {
    const app = await createSearchApp();
    const response = await app.inject({
      method: 'GET',
      url: `/v1/search?q=${creator.toLowerCase()}&limit=10`,
    });
    ...
    expect(response.json()).toMatchObject({
      data: [
        expect.objectContaining({
          tokenAddress: canonicalToken.toLowerCase(),
          deployerAddress: creator.toLowerCase(),
          matchKind: 'CREATOR',
        }),
      ],
    });
    expect((response.json().data as Array<{ tokenAddress: string }>).map((row) => row.tokenAddress)).toEqual([
      canonicalToken.toLowerCase(),
    ]);
