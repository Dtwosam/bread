...
describe("Day 9 post-commit projection cache invalidation publisher", () => {
  it("deduplicates and sorts channels deterministically before invalidating them", async () => {
    ...
    const result = await publish?.({
      ...
      schemaVersion: "bread-projection-v1",
      channels: ["token:5:0xbbb", "stack:5:v1:0xaaa:feed", "token:5:0xbbb"],
    });
  ...
  it("bounds one invalidation batch to at most 128 concurrent Redis increments", async () => {
    const publish = await publisher();
    const channels = Array.from(
      { length: 130 },
      (_, index) => `token:5:0x${index.toString(16).padStart(40, "0")}`,
    );
    let active = 0;
    let maxActive = 0;
  ...
  it("reports DEGRADED after Redis invalidation failure without throwing", async () => {
    const publish = await publisher();
