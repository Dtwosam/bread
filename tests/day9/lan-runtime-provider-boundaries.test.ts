...
describe("Day 9 LAN provider/runtime regressions", () => {
  it("classifies provider rate throttling before generic -32005 request limits", () => {
    expect(classifyArcRpcLimitError(limitError("rate limit exceeded"))).toBe(
      "RATE_LIMIT",
    );
  ...
  it("proactively spaces concurrent Arc reads before the provider has to throttle them", async () => {
    const starts: number[] = [];
    const delays: number[] = [];
    let clock = 0;
    ...
    const raw = {
      getBlockNumber: async () => enter(900n),
      getBlock: async () => enter({ hash: "0xabc", timestamp: 1n }),
      readContract: async () => enter("ok"),
      getLogs: async () => enter([] as readonly RpcLog[]),
    };
  ...
  it("pins one retry owner, proactive pacing and visible LAN catch-up progress", () => {
  ...
  it("plain Node resolves the exact TypeScript helpers used by the operator orchestrator", () => {
    ...
    const lifecycle = pathToFileURL(
      resolve(root, "scripts/day9/lan/process-lifecycle.mts"),
    ).href;
    const proxy = pathToFileURL(
      resolve(root, "scripts/day9/lan/origin-proxy.mts"),
    ).href;
    const result = spawnSync(
      ...
      [
        "--input-type=module",
        "-e",
        `await import(${JSON.stringify(lifecycle)}); await import(${JSON.stringify(proxy)});`,
      ],
