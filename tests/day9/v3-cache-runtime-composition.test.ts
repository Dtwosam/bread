...
describe("Day 9 V3 post-commit cache runtime composition", () => {
  it(
    "routes deterministic <=128-channel chunks through the existing PostCommitPublisher",
    async () => {
  ...
  it(
    "uses PostCommitPublisher invalidation/fanout semantics instead of a second cache authority",
    async () => {
      const publish = await runtimePublisher();
  ...
  it(
    "surfaces degraded post-commit cycles from catch-up without retrying committed projection",
    async () => {
