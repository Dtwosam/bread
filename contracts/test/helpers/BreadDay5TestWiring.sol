...
library BreadDay5TestWiring {
    ...
    function wire(
        ...
        return wire(factory, address(usdc), escrow, address(emergencyController));
    }
    ...
    {
        return wire(factory, address(usdc), escrow, emergencyController);
    }

    function wire(BreadLaunchFactory factory, address usdc, BreadFeeEscrow escrow, address emergencyController)
        internal
        returns (
            BreadPermanentLiquidityLocker locker,
            MockGraduationCoordinator coordinator,
        ...
        )
    {
        ...
        coordinator = new MockGraduationCoordinator(
            address(factory), usdc, address(escrow), emergencyController, address(locker)
        );
        locker.setCoordinator(address(coordinator));
        factory.setGraduationCoordinator(coordinator);

        adapter = new MockGraduationAdapter(
            IGraduationAdapter.AdapterFamily.UNISWAP_V4, usdc, address(locker), CONFIG_HASH
        );
