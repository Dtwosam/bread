...
function holderClauses(bounds: ExploreHolderBounds | undefined) {
  if (!bounds) {
    return { joinClause: sql``, knownClause: sql``, minClause: sql``, maxClause: sql`` } as const;
  }
  return {
...
function marketCapClauses(bounds: ExploreMarketCapBounds | undefined) {
  ...
  return {
    joinClause: sql`INNER JOIN token_metrics market_cap_metric
      ON market_cap_metric.chain_id = launch_scope.chain_id
     AND market_cap_metric.token_address = launch_scope.token_address`,
    knownClause: sql`AND market_cap_metric.market_cap IS NOT NULL`,
    minClause: bounds.minQuote === undefined ? sql`` : sql`AND market_cap_metric.market_cap >= CAST(${bounds.minQuote} AS numeric)`,
    maxClause: bounds.maxQuote === undefined ? sql`` : sql`AND market_cap_metric.market_cap <= CAST(${bounds.maxQuote} AS numeric)`,
  } as const;
}
...
export class TrendingRepository {
  ...
  async listTrendingLaunches(
    ...
    const result = await this.db.execute(sql`
      ...
      metrics AS (
        SELECT token_address, SUM(quote_amount) AS quote_volume_1h,
      ...
      latest AS (
        SELECT DISTINCT ON (token_address) token_address,
      ...
      ranked AS (
        SELECT metrics.token_address, metrics.quote_volume_1h,
          metrics.unique_traders_1h, metrics.trade_count_1h,
      ...
      SELECT
        ...
        l.name, l.symbol, l.metadata, l.quote_asset AS "quoteAsset",
      ...
      WHERE true
        ...
        ${volume24.maxClause}
        ${cursorClause}
      ORDER BY r.quote_volume_1h DESC, r.unique_traders_1h DESC, r.trade_count_1h DESC,
        r.latest_activity_block_number DESC, r.latest_activity_log_index DESC, r.token_address ASC
      LIMIT ${boundedLimit}
    `);
